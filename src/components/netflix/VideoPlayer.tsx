...
function proxyUrl(originalUrl: string): string {
  ...
  // All other provider CDN hosts also go through our server so the
  // minted token and required Referer remain server-side.
  return `/api/proxy?url=${encodeURIComponent(originalUrl)}`;
}
...
function MobilePlayer({ tmdbId, mediaType, season, episode, title, preloadedSources, posterUrl }: PlayerInnerProps) {
  ...
  useEffect(() => {
    ...
    // Auto-fallback advances through all sources in order.
    const nextPlayableIndex = (from: number): number => {
      return from + 1 < sources.length ? from + 1 : -1;
    };
